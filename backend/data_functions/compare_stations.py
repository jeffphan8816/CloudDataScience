...
if __name__ == '__main__':
    ...
    df_es_stations[['longitude', 'latitude']] = pd.DataFrame(
        df_es_stations['location'].tolist(), index=df_es_stations.index)
    ...
    diff1 = df_bom_stations[~df_bom_stations['ID'].isin(
        df_es_stations['Station ID'])]
    diff2 = df_es_stations[~df_es_stations['Station ID'].isin(
        df_bom_stations['ID'])]
    diff1
    diff2

    same1 = df_bom_stations[df_bom_stations['ID'].isin(
        df_es_stations['Station ID'])]
    same2 = df_es_stations[df_es_stations['Station ID'].isin(
        df_bom_stations['ID'])]
    same1
    same2

    # Eg Geelong/Geelong Racecourse
    mask = df_bom_stations.apply(lambda row: row.astype(
        str).str.contains('geelong', case=False).any(), axis=1)
    df_bom_stations[mask]

    mask = df_es_stations.apply(lambda row: row.astype(
        str).str.contains('geelong', case=False).any(), axis=1)
    df_es_stations[mask]
