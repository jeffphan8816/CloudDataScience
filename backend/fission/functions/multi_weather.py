...
def get_weather_data(selected_areas: list) -> pd.DataFrame:
    ...
    with open("./backend/fission/functions/bom_lookup.json", "r") as f:
...
def insert_data_to_es(df: pd.DataFrame):
    ...
    with open("./backend/fission/functions/bom_es_mapping.json", "r") as f:
        mapping = json.load(f)
    ...
    for i, row in df.iterrows():
        es.index(index=index_name, body=row.to_dict(), id=i)
